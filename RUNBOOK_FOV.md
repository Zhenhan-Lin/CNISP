...
- `corrector_data.data_root` → the FOV root (default `nnunet-c/data_fov`),
...
TM=nnunet-c/data_fov/fov_truncation_manifest.json
...
`data_root = nnunet-c/data_fov`. To regenerate the truncated CTs: `rm -rf
nnunet-c/data_fov/images` (or `--force`). Verify the train log shows
