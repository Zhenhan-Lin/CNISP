...
def analyze_case(cnisp_nn: np.ndarray, ref_nn: np.ndarray, visible_box,
    ...
    nnUNet label arrays ({1..4}) on the SAME grid; ``visible_box`` is the retained
    box (per-axis [lo, hi)) in that grid; ``spacing`` is mm/voxel."""
    T = _truncated_mask(cnisp_nn.shape, visible_box)          # blanked FOV
...
def run_analyze(args) -> int:
    ...
    if not info or "visible_box" not in info:
        print(f"[fov-test] no box sidecar entry for source={args.source_id} step={args.step} "
              f"(is this a --mode box build?)", file=sys.stderr)
        return 2
    ...
    d.update({"source_id": args.source_id, "step": args.step,
              "corner": info.get("corner"), "keep_fraction": info.get("keep_fraction"),
              "flag": _flag(d)})

    print(json.dumps(d, indent=2))
    print(f"\n[fov-test] source={args.source_id} step={args.step} corner={info.get('corner')} "
          f"keep={info.get('keep_fraction')}  ->  {d['flag']}")
