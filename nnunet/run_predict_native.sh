...
if f == "all":
    ...
    dice = lambda k: json.loads((mf/f"fold_{k}"/"validation"/"summary.json").read_text())["foreground_mean"]["Dice"]
    sel = [max(avail, key=lambda k: (dice(k) if (mf/f"fold_{k}"/"validation"/"summary.json").is_file() else -1))]
else:
