...
Submodules:
    ...
    * :mod:`nnunet.helpers.paired_csv`  -- readers/filters for
      ...
      ``build_method_summary.py`` and
      ``build_paired_summary.py``.
    * :mod:`nnunet.helpers.patch_size`  -- canonical-align patch-size
      resolver used by ``build_dataset835_canonical_patches.py``
      and ``build_dataset835_sparse_patches.py`` to pin the
